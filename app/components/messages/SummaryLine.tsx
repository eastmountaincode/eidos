...
export function SummaryLine({
  ...
  return (
    <section
      ...
      className="flex min-h-9 flex-wrap items-center justify-between gap-x-5 gap-y-2 rounded-lg border border-border bg-white px-3 py-2"
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-accent bg-accent px-3 py-1.5 text-xs font-bold text-white disabled:cursor-default disabled:opacity-65"
          disabled={isIngesting}
          onClick={onIngest}
          type="button"
        >
          {isIngesting ? <span className="size-3 rounded-full border-2 border-white/45 border-t-white animate-spin" aria-hidden="true" /> : null}
