...
import { Binoculars, BookMarked, BookOpenText, MessageSquareText, Palette, Sparkles, Wrench } from "lucide-react";
...
const navItems = [
  ...
  { href: "/sources", label: "Sources", mobileLabel: "Sources", Icon: BookMarked },
  { href: "/future", label: "Future", mobileLabel: "Future", Icon: Binoculars },
  { href: "/mantra", label: "Mantra", mobileLabel: "Mantra", Icon: Sparkles },
  { href: "/style", label: "Style", mobileLabel: "Style", Icon: Palette },
  ...
  return (
    <div className="min-h-screen bg-bg text-ink md:grid md:h-screen md:grid-cols-[236px_minmax(0,1fr)] md:overflow-hidden">
      <aside className="sticky top-0 z-10 bg-sidebar px-4 py-3 text-white md:h-screen md:overflow-y-auto md:px-3.5 md:py-4">
        ...
        <nav className="mt-3 grid grid-cols-7 gap-1.5 md:mt-5 md:grid-cols-1 md:gap-1">
          {navItems.map((item) => {
