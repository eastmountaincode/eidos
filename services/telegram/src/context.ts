...
export async function buildPrompt(userText: string, profile: ProfileName): Promise<string> {
  ...
  return [
    ...
    '- Style repository: use `python3 ~/.eidos/services/styles/style_context.py --list` to read captured aesthetics, effects, and components from the portal Style page. Save references with `--add --source-text "SOURCE"` plus optional `--kind`, `--url`, `--context`, `--notes`, `--tags`, and `--file-path`.',
